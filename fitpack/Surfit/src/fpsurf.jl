...
"""
(internal) calculate surface splines.
# Arguments:
- iopt:  options see Surfit
- m: length of data points  
- x (vector float): x values
- y (vector float): y values
- z (vector float): z values
- spline:  spline to be calculated
- s: smoothing value
- nxest: upper limit for x knots
- nyest: upperr limit for y knots
- eta :  threshold for determining the effective rank of an 
         over-determined linear system of equations.
         0 < eps < 1. 1e-15 is recommended
       ...
       # b-spline representation of s(x,y).
        ...
        for num in 1:nreg # do 250 num=1,nreg
          ...
          # matrix according to a data point of the panel.
            ...
            in = wrk1.index[num]
            @label L150  
            #if (in == 0) continue end # @goto  L250 end 
            while in != 0
                ...
                irot = jrot
                zi = mkTriangle!(irot, iband, zi, wrk1.h, wrk1.a, wrk1.f)
        ...
        for i in 1:ncof # do 270 i=1,ncof
            if (wrk1.a[i, 1] <= sigma) is_full_rank = false
                break# @goto  L280 
            end
                ...
                for j in 1:nk1x # do 550 j=1,nk1x
                    ...
                    for irot in jrot:ncof# do 540 irot=jrot,ncof
                        ...
                        if (piv == 0.) 
                            ...
                            if (i2 == 0) break end# @goto  L550 end
                ...
                for j in 1:nk1y # do 630 j=1,nk1y
                    ...
                    for irot in jrot:ncof# do 620 irot=jrot,ncof
                        ...
                        if (piv == 0.) 
                            ...
                            # apply that givens transformation to the right hand side.
                            zi, wrk1.ff[irot] =fprota(v_cos, v_sin, zi, wrk1.ff[irot])
                            if (i2 == 0) break end # @goto  L630 end
                            # apply that givens transformation to the left hand side.
...
"""Rotate row into triangle.
for the equation 
    A * h = f

    Args:
    irot (Int)  : row to rotate
    iband (int) : bandwidth of a
    z (float): z value data point times weight
    h (vactor float): lhs vector
    a (matrix float): lhs array
    f (vector float): rhs vector

    modifies h, a and f
    return new zi
"""
function mkTriangle!(irot, iband, zi, h::Vector, a::Matrix, f::Vector)
   
    for i in 1:iband # do 220 i=1,iband
        irot = irot + 1
        piv = h[i]
        if (piv == 0.0) continue end # @goto  L220 end
        # calculate the parameters of the givens transformation.
        v_cos, v_sin, a[irot, 1] = fpgivs(piv, a[irot, 1])
        # apply that transformation to the right hand side.
        zi, f[irot] = fprota(v_cos, v_sin, zi, f[irot])
        if (i == iband) break end # @goto  L230 end
        # apply that transformation to the left hand side.
        i2 = 1
        i3 = i + 1
        for j in i3:iband # do 210 j=i3,iband
            i2 = i2 + 1
            h[j], a[irot, i2] = fprota(v_cos, v_sin, h[j], a[irot, i2])
